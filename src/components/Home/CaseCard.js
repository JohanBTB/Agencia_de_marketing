...
export default function CaseCard({data, index}) {
    ...
    return (
        ...
        <h3 class="mt-5 text-2xl font-semibold leading-6 text-mydarkblue-900 " id={index}>
            <a href={data.href}>
            <span class="absolute inset-0" ></span>
            {data.title}
            </a>
        </h3>
        <p class="mt-5 text-xl leading-8 text-myblue-900 line-clamp-3">{data.content}</p>
        </div>
        ...
        <time datetime="2020-03-16" class=" text-lg text-mylightblue-500">{data.date}</time>
